    ...
    public static void Main(string[] args)
    {
        try
        {
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                // 正規表現を使ってURLを抽出
                MatchCollection matches_beta = Regex.Matches(htmlContent, @"""(https://hide\.maruo\.co\.jp/software/bin3/hm\d+b\d+_signed\.exe)""");

                // URLのリストを作成する
                List<string> urls = new List<string>();
                foreach (Match match in matches_beta)
                {
                    urls.Add(new Uri(new Uri("https://hide.maruo.co.jp"), match.Groups[1].Value).AbsoluteUri);
                }
